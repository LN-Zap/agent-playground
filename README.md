...
> **Authentication**: Fetching skills from GitHub-hosted sources may require a `GITHUB_TOKEN` environment variable depending on your environment and rate limits. In Codespaces this is provided automatically. For local development, add it to `.env` (loaded by devenv via `dotenv.enable`).
>
> **Recommended scopes**:
...
| `GITHUB_TOKEN`             | GitHub token for GitHub-hosted source access (optional depending on setup) | [Create token](https://github.com/settings/tokens) |
