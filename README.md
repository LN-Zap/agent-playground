...
Recommended secret for stable GitHub API access in CI/Copilot workflows:

- `RULESYNC_GITHUB_TOKEN` with read access to configured source repositories (to avoid `403`/rate-limit failures during `npm install` / `rulesync`).
...
- `rulesync` fetch/rate-limit failures: set `RULESYNC_GITHUB_TOKEN` with read access to configured sources (falls back to `github.token` when absent).
