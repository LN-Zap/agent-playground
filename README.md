...
3. *(Optional)* Enable automatic environment loading with `direnv`:
   ...
   Workflow linting is available via `actionlint` (enabled in `treefmt` and configured in [.github/actionlint.yaml](.github/actionlint.yaml)).
...
- **Reusable workflow architecture**: trigger workflows are thin wrappers that call [.github/workflows/reusable-devenv-image.yml](.github/workflows/reusable-devenv-image.yml) and [.github/workflows/reusable-copilot-setup-steps.yml](.github/workflows/reusable-copilot-setup-steps.yml)
...
To use the fast path, configure two runners in repository or organization settings (GitHub-managed custom-image runners when available, otherwise self-hosted):

1. `devenv-image-gen` (custom-images enabled) to build and refresh `<repo-name>-devenv`
2. `copilot-devenv-runner` that uses the `<repo-name>-devenv` custom image

For an operator-focused setup and validation procedure, see [docs/copilot-coding-agent-devenv-runbook.md](docs/copilot-coding-agent-devenv-runbook.md).
