...
- **Primary path (fast start)**: use a custom runner image generated by [.github/workflows/devenv-image.yml](.github/workflows/devenv-image.yml) via `snapshot: copilot-devenv`
- **Fallback path**: if `devenv` is not present on the runner, setup installs Nix + devenv inline, restores `/nix/store` cache, and then exports devenv environment variables for agent steps
...
1. `devenv-image-gen` (custom-images enabled) to build and refresh `copilot-devenv`
2. `copilot-devenv-runner` that uses the `copilot-devenv` custom image
